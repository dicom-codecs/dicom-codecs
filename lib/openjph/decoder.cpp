...
static dicomcodecs::size
calculateSizeAtDecompositionLevel(const dicomcodecs::image &image,
...
void openjph_decoder(const vector<uint8_t> &encodedBytes, image &targetImage) {
  ...
  const size_t destinationSize = sizeAtDecompositionLevel.width *
                                 sizeAtDecompositionLevel.height *
                                 targetImage.componentCount * bytesPerPixel;
  targetImage.rawBytes.resize(destinationSize);

  // set the level to read to and reconstruction level to the specified
  // decompositionLevel
  ...
  // NOTE: All values must be clamped
  // https://github.com/aous72/OpenJPH/issues/35
  ojph::ui32 comp_num;
  for (int y = 0; y < sizeAtDecompositionLevel.height; y++) {
    size_t lineStart = y * sizeAtDecompositionLevel.width *
                       targetImage.componentCount * bytesPerPixel;
    if (targetImage.componentCount == 1) {
      ...
      for (int c = 0; c < targetImage.componentCount; c++) {
        ...
        if (targetImage.bitsPerSample <= 8) {
          ...
          for (size_t x = 0; x < sizeAtDecompositionLevel.width; x++) {
            int val = line->i32[x];
            pOut[x * targetImage.componentCount] =
                std::max(0, std::min(val, UCHAR_MAX));
          }
        } else {
          ...
          if (targetImage.isSigned) {
            ...
            for (size_t x = 0; x < sizeAtDecompositionLevel.width; x++) {
              int val = line->i32[x];
              pOut[x * targetImage.componentCount] =
                  std::max(SHRT_MIN, std::min(val, SHRT_MAX));
            }
          } else {
            unsigned short *pOut =
                (unsigned short *)&targetImage.rawBytes[lineStart] + c;
            for (size_t x = 0; x < sizeAtDecompositionLevel.width; x++) {
              int val = line->i32[x];
              pOut[x * targetImage.componentCount] =
                  std::max(0, std::min(val, USHRT_MAX));
            }
          }
        }
      }
