...
void openjph_encoder(const image &sourceImage, vector<uint8_t> &encodedBytes) {
    ...
    for (int c = 0; c < num_comps; ++c) {
        siz.set_component(c, ojph::point(downSamples_[c].x, downSamples_[c].y), sourceImage.bitsPerSample, sourceImage.isSigned);
    }
    ...
    // Encode the image
    const size_t bytesPerPixel = sourceImage.bitsPerSample / 8;
    ojph::ui32 next_comp;
    ...
    for (size_t y = 0; y < height; y++)
    {
      for (size_t c = 0; c < siz.get_num_components(); c++)
      {
        ...
        {
          uint8_t *pIn = (uint8_t *)(sourceImage.rawBytes.data() + (y * sourceImage.width * bytesPerPixel * siz.get_num_components()) + c);
        ...
        else
        {
          ...
          {
            int16_t *pIn = (int16_t *)(sourceImage.rawBytes.data() + (y * sourceImage.width * bytesPerPixel));
            for (size_t x = 0; x < sourceImage.width; x++)
            {
              *dp++ = *pIn++;
            }
          ...
          {
            uint16_t *pIn = (uint16_t *)(sourceImage.rawBytes.data() + (y * sourceImage.width * bytesPerPixel));
            for (size_t x = 0; x < sourceImage.width; x++)
            {
              *dp++ = *pIn++;
            }
