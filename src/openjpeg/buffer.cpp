...
static OPJ_SIZE_T opj_read_from_buffer(void *pdst, OPJ_SIZE_T len,
...
static OPJ_SIZE_T opj_write_to_buffer(void *p_buffer, OPJ_SIZE_T p_nb_bytes,
  ...
  const OPJ_BYTE *pbuf = p_source_buffer->buf;
  const OPJ_BYTE *pcur = p_source_buffer->cur;
...
static OPJ_SIZE_T opj_skip_from_buffer(OPJ_SIZE_T len,
                                       opj_buffer_info_t *psrc) {
  OPJ_SIZE_T n = psrc->buf + psrc->len - psrc->cur;
