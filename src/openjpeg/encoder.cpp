...
void openjpegencoder(const dicomcodecs::image& sourceImage, std::vector<uint8_t> & encodedBytes) {
    ...
    OPJ_COLOR_SPACE color_space = sourceImage.componentCount > 1 ? OPJ_CLRSPC_SRGB : OPJ_CLRSPC_GRAY;
    ...
    image = opj_image_create((OPJ_UINT32)sourceImage.componentCount, cmptparm.data(), color_space);

    /* set image offset and reference grid */
    image->x0 = (OPJ_UINT32)imageOffset_.x;
    image->y0 = (OPJ_UINT32)imageOffset_.y;
    image->x1 = (OPJ_UINT32)sourceImage.width; // TODO: revisit logic in terms of subsampling and offsets?
    image->y1 = (OPJ_UINT32)sourceImage.height; // TODO: revisit logic in terms of subsampling and offsets?

    if(sourceImage.bitsPerSample <= 8) {
      if(sourceImage.componentCount == 1) {
        std::copy((uint8_t*)sourceImage.rawBytes.data(), (uint8_t*)(sourceImage.rawBytes.data() + sourceImage.rawBytes.size()), image->comps[0].data);
      } else {
        for(size_t compno = 0; compno < sourceImage.componentCount; compno++) {
          for(size_t i=0; i < sourceImage.width * sourceImage.height; i++) {
            image->comps[compno].data[i] = sourceImage.rawBytes.data()[(i * sourceImage.componentCount) + compno];
          }
        }
      }
    } else if(sourceImage.bitsPerSample <= 16) {
      if(sourceImage.isSigned) {
        std::copy((short*)sourceImage.rawBytes.data(), (short*)(sourceImage.rawBytes.data() + sourceImage.rawBytes.size()), image->comps[0].data);
      } else {
        std::copy((unsigned short*)sourceImage.rawBytes.data(), (unsigned short*)(sourceImage.rawBytes.data() + sourceImage.rawBytes.size()), image->comps[0].data);
      }
    }

    /* set encoding parameters to default values */
    opj_set_default_encoder_parameters(&parameters);
    parameters.tcp_mct = (char)sourceImage.componentCount > 1 ? 1 : 0; // disable for grayscale: TODO - set this properly for color
