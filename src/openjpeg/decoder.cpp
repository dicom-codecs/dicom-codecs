...
#include "buffer.hpp"
  ...
  dicomcodecs::size calculateSizeAtDecompositionLevel(const dicomcodecs::image& imageFrame, int decompositionLevel) {
...
void openjpegdecoder(const std::vector<uint8_t> & encodedBytes, dicomcodecs::image& targetImage) {
    ...
    opj_dparameters_t parameters;
      ...
      const size_t destinationSize = targetImage.width * targetImage.height * targetImage.componentCount * bytesPerPixel;
      ...
      for (int y = 0; y < sizeAtDecompositionLevel.height; y++)
      {
        ...
        size_t lineStart = lineStartPixel * targetImage.componentCount * bytesPerPixel;
